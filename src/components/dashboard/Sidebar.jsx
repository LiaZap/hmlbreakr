...
const Sidebar = ({ activePage = 'home', onNavigate, isOwner = true, lockCollapsed = false }) => {
  ...
  if (isOwner) {
    navItems.push(
      ...
      { id: 'assinatura', label: 'Assinatura', icon: (active) => (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
          <rect x="3" y="6" width="18" height="13" rx="2" stroke={active ? '#F5A623' : '#959387'} strokeWidth="1.5"/>
          <path d="M3 10h18" stroke={active ? '#F5A623' : '#959387'} strokeWidth="1.5"/>
          <path d="M7 15h4" stroke={active ? '#F5A623' : '#959387'} strokeWidth="1.5" strokeLinecap="round"/>
        </svg>
      )}
    );
  }
