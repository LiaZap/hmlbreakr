/**
 * BrazilMap — Distribuição geográfica do portfólio de restaurantes.
 *
 * ⚠️ DESATIVADO (ticket BAH-093, 2026-05-15)
 * ───────────────────────────────────────────────────────────────────────
 * Este componente foi DESATIVADO porque o sistema NÃO coleta a UF/estado
 * do restaurante em lugar nenhum — nem no onboarding (`location_costs` só
 * tem rent/IPTU), nem em qualquer outro fluxo. Sem fonte de dado, o mapa
 * indexava 0 clientes e a seção "Distribuição Geográfica" aparecia sempre
 * vazia ("X sem UF").
 *
 * Por isso `BrazilMap` agora retorna SEMPRE `null` — assim a seção some
 * automaticamente do render do AdminPanel sem precisar editar AdminPanel.jsx.
 *
 * REATIVAÇÃO FUTURA: se o onboarding passar a coletar UF (ex.: campo
 * `formData.location_costs.state` ou `formData.identity.state`), basta
 * descomentar a IMPLEMENTAÇÃO ORIGINAL preservada no fim deste arquivo
 * (bloco em comentários `//`), remover este componente-stub e os imports
 * `useMemo`/`useState`/`motion`/`computeClientHealth` voltam a ser usados.
 * Toda a lógica de extração de UF, bucket por região e bubble map está
 * intacta — nada foi apagado.
 * ───────────────────────────────────────────────────────────────────────
 */

// eslint-disable-next-line no-unused-vars
const BrazilMap = ({ clients = [], onClientClick }) => {
  // Desativado por ausência de coleta de UF — ver cabeçalho (ticket BAH-093).
  return null;
};
...
// ═══════════════════════════════════════════════════════════════════════
// IMPLEMENTAÇÃO ORIGINAL PRESERVADA (reativar quando houver coleta de UF).
// Mantida como comentários de linha (`//`) — não usar bloco /* */ porque
// o código original contém JSDoc com `*/` e quebraria o comentário.
// ═══════════════════════════════════════════════════════════════════════
//
...
//
//   // 1-3: campos estruturados (futuro)
//   const direct =
...
// };
//
// // Componente principal (versão antiga, ativa quando houver coleta de UF)
// const BrazilMapImpl = ({ clients = [], onClientClick }) => {
