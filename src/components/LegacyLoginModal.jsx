 ...
 * NAO foram migrados pro Clerk.
 *
 * Contexto (29/05/2026): apos a atualizacao recente que fez todos novos
 * clientes serem criados automaticamente no Clerk, alguns clientes
 * existentes (criados ANTES da atualizacao) ficaram sem entrada no
 * Clerk e nao conseguem logar pelo widget <SignIn>. Esse modal e o
 * caminho legado pra eles continuarem acessando ate a migracao em massa
 * via scripts/migrate-users-to-clerk.js ser executada.
 *
 * Fluxo:
 *   1. Cliente clica no link "Cliente antigo? Entre aqui" embaixo do widget
 *   2. Digita email + senha
 *   3. POST /api/client/login (mesmo endpoint do AdminLoginTab — bcrypt)
 *   4. Em sucesso → onLogin(hash) faz a navegacao
 *
 * Quando todos forem migrados pro Clerk, basta remover esse arquivo
 * e o botao no ClientLogin.jsx — zero impacto no backend (endpoint
 * /api/client/login continua sendo usado pela aba Admin oculta).
 */
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#868686] mt-0.5">Conta criada antes da migração para Clerk</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-5">
          <div className="bg-[#5B8DEF]/[0.08] border border-[#5B8DEF]/30 rounded-[10px] p-3 mb-4">
            <p className="text-[11px] text-[#CFCFCF] leading-relaxed">
              Se você já tinha conta no Breakr antes da nossa atualização de autenticação,
              entre aqui com seus dados antigos. Sua conta será migrada automaticamente em breve.
            </p>
          </div>
