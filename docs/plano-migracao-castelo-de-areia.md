...
- [x] **Custos (6 listas + objetos de custo)**: flag `Client.readCustosFromTables` (default OFF). `coreRead.reconstructCustos` (6 listas Employee/Partner/Equipment/Vehicle/CardMachine/Marketplace) + `reconstructCostObjects` (FixedCostItem → `formData.{location_costs,utilities,recurring_services,operational_fixed,admin_systems,marketing_structure,monthly_services,other_fixed_costs}`). Round-trip `f3-roundtrip-custos.js` **100% fiel** (456 itens de lista + **466 chaves de custo + 75 itens de array**, 0 divergência). `partner.photo` (base64) = fallback do blob. Injeção = `Object.assign` (substitui as 6 listas + 8 objetos de custo).
  - ✅ **Gap dos objetos de custo FECHADO** (mig. `0008`): `FixedCostItem` virou **espelho fiel** (`rawValue` text = string original; `position` p/ arrays; `amount` nullable; guarda **TODAS** as chaves incl. zeros e config string como `systems_count`/`ads_platform`/`gifts_*`). Antes era agregado EAV lossy.
